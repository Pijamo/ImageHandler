...
class ImageRequest {
     ...
     */
    determineOutputFormat(imageRequestInfo, event) {
    ...
    fixQuality(imageRequestInfo) {
        if (imageRequestInfo.outputFormat) {
            ...
            if (requestType.includes(imageRequestInfo.requestType) &&
    ...
    setup(event) {
        return __awaiter(this, void 0, void 0, function* () {
            console.warn("In the Image Request Setup function")
            try {
                ...
                console.warn("Edits: ", imageRequestInfo.edits);
                ...
                if (imageRequestInfo.contentType === lib_1.ContentTypes.SVG &&
    ...
    parseRequestType(event) {
        ...
        const matchThumbor = /^(\/?)((fit-in)?|(filters:.+\(.?\))?|(unsafe)?)(((.(?!(\.[^.\\/]+$)))*$)|.*(\.jpg$|\.jpeg$|.\.png$|\.webp$|\.tiff$|\.tif$|\.svg$|\.gif$))/i;
        const { REWRITE_MATCH_PATTERN, REWRITE_SUBSTITUTION } = process.env;
        const definedEnvironmentVariables = REWRITE_MATCH_PATTERN !== "" &&
    ...
    getOutputFormat(event, requestType = undefined) {
        ...
        if (AUTO_WEBP === "Yes" && event.headers.Accept && event.headers.Accept.includes(lib_1.ContentTypes.WEBP)) {
    ...
    inferImageType(imageBuffer) {
        ...
        switch (imageSignature) {
            case "89504E47":
                return lib_1.ContentTypes.PNG;
            case "FFD8FFDB":
            case "FFD8FFE0":
            case "FFD8FFEE":
            case "FFD8FFE1":
                return lib_1.ContentTypes.JPEG;
            case "52494646":
                return lib_1.ContentTypes.WEBP;
            case "49492A00":
            case "4D4D002A":
                return lib_1.ContentTypes.TIFF;
            case "47494638":
                return lib_1.ContentTypes.GIF;
            default:
